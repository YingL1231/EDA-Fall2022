...
BRA_complete.rainfall_ts <- ts(BRA_complete$Rainfall, start = c(1901,1,1), frequency = 12)

BRA_complete.temp_ts <- ts(BRA_complete$Temperature_C, start = c(1901,1,1), frequency = 12)
```
...
BRA_complete.rainfall_Decomposed <- stl(BRA_complete.rainfall_ts, s.window = "periodic")

BRA_complete.temp_Decomposed <- stl(BRA_complete.temp_ts, s.window = "periodic")
...
BRA_complete.rainfall_Components <- as.data.frame(BRA_complete.rainfall_Decomposed$time.series[,1:3])

BRA_complete.rainfall_Components <- mutate(BRA_complete.rainfall_Components,
        Observed = BRA_complete$Rainfall,     
        Date = BRA_complete$Date)

BRA_complete.temp_Components <- as.data.frame(BRA_complete.rainfall_Decomposed$time.series[,1:3])

BRA_complete.temp_Components <- mutate(BRA_complete.temp_Components,
        Observed = BRA_complete$Temperature_C,     
        Date = BRA_complete$Date)

# Exercise 10: Visualize how the trend maps onto the data for both series
ggplot(BRA_complete.rainfall_Components) +

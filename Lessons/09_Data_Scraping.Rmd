...
```{r whatif we scrape month}
avg_withdrawals_month <- webpage %>% 
  html_nodes('.table:nth-child(7) th , .table:nth-child(7) td') %>%
  html_text()
avg_withdrawals_month

df_withdrawals_month <- data.frame(
...
df_withdrawals_month <- df_withdrawals_month %>% 
...
df_withdrawals <- data.frame("Month" = rep(1:12),#add month column
                             "Year" = rep(2000,12),
                             "Avg_Withdrawals_mgd" = as.numeric(avg_withdrawals),
                             "Max_Withdrawals_mgd" = as.numeric(max_withdrawals))
...
#OR, use purrr's map function(tidyverse)
...
ggplot(the_df,aes(x=Date,y=Avg_Withdrawals_mgd)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
  labs(title = paste(" Water usage data for",the_registrant),
       subtitle = the_facility_name,
       y="Withdrawal (mgd)",
       x="Date")
```

# Part 3. Web Crawling:get data from different sites
